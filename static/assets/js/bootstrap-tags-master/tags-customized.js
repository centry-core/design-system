...
class BootstrapInputsTags {
  ...
  _configureSearchInput() {
    ...
    this._searchInput.style.cssText = `backgroundColor:transparent;color:currentColor;border:0;padding:4px 0 4px 4px;outline:0;maxWidth:100%`;
  ...
  _buildSuggestionsItem(suggestion, i) {
    ...
    const newChild = document.createElement("li");
    newChild.setAttribute("role", "presentation");
    newChild.classList.add("dropdown-item__li");
    if (suggestion.group_id) {
      newChild.setAttribute("data-group-id", "" + suggestion.group_id);
    }
    const newChildLink = document.createElement("a");
    const newChildText = document.createElement("span");
    if (suggestion.data.styleColor) {
      newChildText.style.color = suggestion.data.styleColor;
      newChildText.style.borderColor = suggestion.data.styleColor;
      newChildText.style.borderWidth = "1px";
      newChildText.style.borderStyle = "solid";
    }
    ...
    newChildText.innerHTML = textContent;
    newChildLink.appendChild(newChildText);
    this._dropElement.appendChild(newChild);
  ...
  removeLastItem(noEvents = false) {
    let items = this._containerElement.querySelectorAll("span.badge:not(.disabled)");
    if (!items.length) {
      return;
    }
    ...
    if (allowClear) {
      ...
      const btn =
          bver === 5
              ...
              : '<button type="button"' +
              btnFloat +
              'text-shadow:none;color:currentColor;" class="' +
              ' close" aria-label="' +
              this._config.clearLabel +
              '"><span aria-hidden="true" class="ml-1"><i class="icon__16x16 icon-close__16"></i></span></button>';
      html = btn + html;
    }
