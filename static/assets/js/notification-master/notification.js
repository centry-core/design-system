function Notification(opts) {
  ...
  const showPopup = ({ type, title, message, callback = null, iconUrl = null } = {}) => {
    opts.type = type;
    ...
    if (iconUrl) {
      const elIcon = elPopup.querySelector(iconSelector);
      let iconImg = document.createElement("img")
      iconImg.src = iconUrl;
      elIcon.append(iconImg);
    }
  ...
  const basic = ({ title, message, iconUrl }) =>
    showPopup({ type: 'basic', title, message, iconUrl });
  return { dialog, info, success, warning, error, setPosition, basic };
}
